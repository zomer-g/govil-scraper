"""
Gov.il Scraper — Flask Application (canonical location).

This module is the live Flask app. The top-level `app.py` at the repo root
re-exports `app` from here so the Render entry `gunicorn app:app` keeps
working unchanged. New code should target `govscraper.web.app:app` directly.

Routes, background job management, SSE progress streaming all live below.
A blueprint extraction (web/routes_*.py) is a future cleanup pass — the
mechanical move from root only fixes the location, not the structure.
"""
...
from storage import CollectionStore  # storage.py at repo root, unmoved
...
from govscraper.io.archive_engine import (
    ...
    run_incremental as _archive_incremental,
)
...
# Templates and static files now live under govscraper/web/, sibling to this
# module. Flask's default resolver (relative to __name__) finds them
# automatically — but we still pass them explicitly so the resolution is
